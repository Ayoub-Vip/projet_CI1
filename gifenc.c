...
void
ge_draw_line(ge_GIF *gif, int x0, int y0, int x1, int y1, uint8_t c) {

  if (gif == NULL) {
    fprintf(stderr,"Passed NULL gif to ge_draw_line()\n");
    return;
  }

  if (x0 < 0 || x0 > gif->w) {fprintf(stderr, "ERROR : invalid x0 value in ge_draw_line()"); return;}
  if (x1 < 0 || x1 > gif->w) {fprintf(stderr, "ERROR : invalid x1 value in ge_draw_line()"); return;}
  if (y0 < 0 || y0 > gif->h) {fprintf(stderr, "ERROR : invalid y0 value in ge_draw_line()"); return;}
  if (y1 < 0 || y1 > gif->h) {fprintf(stderr, "ERROR : invalid y1 value in ge_draw_line()"); return;}
